...
- 높은 병렬성으로 공유된 변수를 도입해 독점적인 락을 사용하는 부분을 줄이는 것
	- 병렬 컬렉션 클래스를 사용하거나, 읽기/쓰기 락을 사용하거나 불변 객체를 사용하고 단일 연산 변수를 사용하는 방법
- ReadWriteLock 클래스를 사용하면 여러개의 Reader와 하나의 Writer가 있는 상황 문제를 압축할 수 있음
- 여러개의 스레드는 값을 읽기만 할 수 있고, 값을 변경 할 수 있는 하나의 스레드만 값을 쓸때 락을 독점적으로 확보함
- 단일 연산 변수(atomic variable)를 사용하면 통계 값을 위한 카운터 변수나 일련번호 생성 모듈 같은 핫 필드가 존재할 때 핫 필드의 값을 손쉽게 변경할 수 있게 해줌
- 핫필드를 단일 연산 변수로 변경한는 것만으로도 확장성에 이득을 볼 수 있음

### 11.4.6 CPU 활용도 모니터링
- 애플리케이션의 확장성을 테스트할 때 그 목적은 대부분 CPU를 최대한 활용하는 데 있음
- 2개 이상의 CPU에서 일부 CPU만 열심히 일하고 있다면 병렬성을 높이는 방법을 찾아 적용해야 함
- 특정 CPU만 열심히 일하는 경우는 상당 부분의 연산 작업이 특정 스레드에서만 일어난다는 것을 뜻함
	- CPU가 여러개 장착된 하드웨어를 애플리케이션에서 충분히 활용하지 못한다고 볼 수 있음
- CPU를 충분히 활용하지 못하고 있다면, 일반적인 몇가지 원인을 생각해 볼 수 있음
	- **부하가 부족하다**
		- CPU 사용량을 측정할 만큼 충분한 부하 상황을 만들지 않을 것일 수 있음
		- 부하를 점점 늘려가며 CPU 사용률을 비롯한 응답 시간, 서비스 시간 등의 항목이 어떻게 증가하는지를 측정할 수 있음
	- **I/O 제약**
		- iostat 이나 perfmon 등의 유틸리티를 사용하여 애플리케이션 중 디스크 관련 부분이 얼마나 되는지 알 수 있음
		- 네트웍의 트래픽 수준을 모니터링하면 대역폭을 얼마나 사용하고 있는지도 쉽게 파악할 수 있음
	- **외부 제약 사항**
		- 애플리케이션에서 외부 데이터베이스 또는 웹 서비스 등을 사용하고 있다면 성능의 병목이 외부에 있을 가능성이 존재
		- 외부적인 부분은 프로파일러를 활용하거나 데이터베이스 모니터링 도구를 사용하여 시간 소요량을 확인 가능함
	- **락 경쟁**
		- 프로파일링 도구를 활용하면 락 경쟁 조건의 발생 빈도와 가장 빈번한 경쟁 락이 어느 것인지 알 수 있음
		- 랜덤 샘플링, 특정 시점의 스레드 덤프를 이용하여 락을 확보하기 위해 경쟁중인 스레드가 어느정도인지 파악 가능
		- 특정 스레드가 락을 확보하기 위해 대기 중이라면 "waiting to lock monitor..."의 메시지가 표시됨
- CPU 사용량이 지속적으로 높게 유지되면서 남는 CPU가 나타나기를 기다리는 스레드가 많아지면 CPU를 더 장착하여 성능을 높일 수 있음

### 11.4.7 객체 풀링은 하지 말자
- **과거** 객체 할당/제거 작업이 늘렸을 때 사용하지 않는 객체를 GC에 넘기지 않고 재사용할 수 있도록 보관해두고, 꼭 필요한 경우에만 새로운 객체를 생성하는 객체 풀을 활용함
- 객체 풀은 GC 소모 시간을 줄여주지만, 단일 스레드 애플리케이션에서 아주 무겁고 큰 객체를 제외하고는 일반적으로 성능에 좋지 않음
	- 크기가 작은 객체를 풀로 관리하는 일은 오히려 자원을 많이 소모함
- 객체 풀이 성능을 최적화할 수 있는 방법이라 생각하지만, 확장성에는 심각한 문제를 일으킴
- 객체 풀은 그것만의 적절한 용도가 있으며, 성능을 최적화하는데 사용하기에는 적절한 방법이 아님
- 스레드 동기화하는 것보다 메모리에 객체를 할당하는 일이 훨씬 부담이 적음

</br>

## 11.5 예제: Map 객체의 성능 분석
- 단일 스레드 환경에서 ConcurrentHashMap은 동기화된 HashMap보다 약간 성능이 빠름
- 병렬 처리 환경에서는 ConcurrentHashMap의 성능이 좋음
- ConcurrentHashMap에서 가장 많이 사용하는 기능이 현재 맵 내부에 갖고 있는 값을 찾아내 가져가는 연산이라고 가정함
	- 여러 개의 스레드에서 get 메소드를 연달아 호출하는 경우에 가장 빠른 속도를 냄
- 대부분의 읽기 연산에는 락을 걸지 않고 있으며, 쓰기 연산과 일부 읽기 연산에는 락 스트라이핑을 활용하고 있음
![map-ex01](/img/map-ex01.png)     
  
- 멀티스레드 환경에서 사용하는 것을 목표로 만들어짐 : ConcurrentHashMap, ConcurrentSkipListMap
- 동기화에 초점을 맞춤 : synchronizedMap, synchronizedSkipListMap
- ConcurrentHashMap, ConcurrentSkipListMap 은 스레드 수가 늘어남에 따라 성능이 잘 따라옴
	- 처리량도 함께 늘어남
- synchronizedMap으로 동기화된 맵은 경쟁 조건이 발생하지 않는 상황에서 경쟁 조건이 발생하는 상황으로 넘어가면 성능이 급격하게 저하됨
	- 락 경쟁을 제대로 막지 못하는 경우에 발생함
	- 한 번 경쟁이 발생하기 시작하면 연산에 필요한 시간의 대부분이 컨텍스트 스위칭과 스케줄링에 필요한 대기 시간으로 소모되며, 스레드를 추가한다해도 성능을 끌어올리지 못함

</br>

## 11.6 컨텍스트 스위치 부하 줄이기
- 실행과 대기, 두 가지 상태를 옮겨 다니는 것을 컨텍스트 스위치라함
- 특정 스레드가 락으 ㄹ확보한 상태에서 I/O 연산이 끝날 때까지 대기상태에 들어가면 실행 중인 다른 스레드가 이미 누군가 확보하고 있는 락을 필요로 할 가능성이 높음
- 락을 놓고 경쟁하고 있다는 말은 컨텍스트 스위치가 많이 일어나고 있다는 말과 같음
	- 병렬 처리 시스템은 대부분의 락을 대상으로 경쟁이 발생하지 않는 경우에 훨씬 높은 성능을 보여줌
- 스레드 입장에서는 대기 상태에 들어가거나 컨텍스트 스위치가 일어나는 일이 원래 작업을 처리하는데 상당한 방해가 됨
