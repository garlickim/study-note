...
- 데드락 상태를 검출하고 데드락에서 복구하는 또 다른 방법은 Lock 클래스의 메소드 가운데 시간을 제한할 수 있는 tryLock 메소드를 사용하는 방법
	...
	- Lock 클래스 등의 명시적인 락은 일정시간을 정해두고 락을 확보하지 못하면 tryLock 메소드가 오류를 발생시키도록 할 수 있음
...
- 데드락이 발생했을 때는 JVM이 만들어 내는 스레드 덤프를 활용하여 데드락이 발생한 위치를 확인하는데 도움을 얻을 수 있음
- 스레드 덤프에는 실행중인 모든 스레드의 스택 트레이스가 담겨 있음
- 스레드 덤프에는 락과 관련된 정보도 담겨 있는데, 각 스레드마다 어떤 락을 확보하고 있는지, 스택의 어느 부분에서 락을 확보했는지, 그리고 대기중인 스레드가 어느 락을 확보하려고 대기 중이었는지 등에 대한 정보를 갖고 있음
- 자바 6에서는 명시적인 Lock을 사용해도 스레드 덤프에 포함될 뿐 아니라 데드락을 검축할 때 명시적인 락을 포함하는 데드락도 검출해줌
- 락을 어디에서 확보했는지에 대해 출력되는 정보는 암묵적인 락에 대한 내용만큼 정확하지는 않음
- 암묵적인 락은 락을 확보하는 시점의 스택 프레임에 연결돼 있지만, 명시적인 락은 락을 확보한 스레드와 연결되어 있기 때문

![deadlock](/img/deadlock.png)    

- 데드락 오류를 발생시킨 원인 : J2EE 애플리케이션, J2EE 컨테이너, JDBC 드라이버
- 실제로 JVM은 데드락을 추적하는데 도움이 되는 정보를 주기 위해 많은 내용을 줌
	- 문제점을 일으킨 락이 어느 것인지
	- 어느 스레드가 관련돼 있는지
	- 관련된 스레드에서 확보하고 있는 다른 락에는 어떤 것이 있는지
	- 문제점으로 인해 다른 스레드가 간접적으로라도 불편함을 겪고 있는지
- 스레드 하나가 MumbleDBConnection에 대한 락을 확보하고 있고, 동시에 MumbleDBCallableStatement 에 대한 락을 확보하려고 대기
- 다른 스레드는 MumbleDBCallableStatement 락을 확보한 상태로 MumbleDBConnection에 대한 락을 확보하려고 대기
- JDBC 드라이버에 락 순서가 일정하지 않다는 버그가 존재한다는 것을 알 수 있음


## 10.3 그 밖의 활동성 문제점
- 병렬 프로그램을 작성하다 보면 소모(starvation), 놓친 신호, 라이브락(livelock) 등과 같이 다양한 원인을 마주치게 됨

</br>

### 10.3.1 소모
- 소모 상태는 스레드가 작업을 진행하는데 꼭 필요한 자원을 영영 할당받지 못하는 경우에 발생함
- 소모 상태를 일으키는 가장 흔한 원인은 바로 CPU
- 자바 애플리케이션에서 소모 상황이 발생하는 원인은 대부분 스레드 우선순위를 적절치 못하게 올리거나 내리는 부분에 있음
	- 또는, 락을 확보한 채로 종료되지 않는 코드를 실행할 때, 다른 스레드에서 해당 락을 가져갈 수 없기 때문에 발생하기도 함
- 자바의 스레드 API는 총 10단계의 스레드 우선 순위를 지정할 수 있도록 되어있으며, 우선순위를 하위 운영체제의 스케줄링 우선 순위에 적절히 대응시키는 정도만 사용함
- 운영체제의 스레드 스케쥴러는 자바 언어 명세(Java Language Specification)에서 명시하고 있는 스레드 스케줄링의 공평성(fairness)과 활동성(liveness)을 지원하기 위해 여러가지 방법을 사용함
- 자바 애플리케이션 내부에서 동작하는 모든 스레드는 Thread.NORM_PRIORITY 값으로 같은 우선순위로 동작함
- 스레드의 우선 순위를 위로 올린다고 해도 아무런 변화가 없거나, 아니면 우선 순위가 높은 스레드만 우선적으로 실행시켜 다른 스레드가 제대로 실행되지 못하게 될 수 도 있음
- 일반적인 상황에서는 스레드 우선 순위를 변경하지 않고 그대로 사용하는 방법이 가장 현명함
- 스레드 우선 순위를 변경하고 싶다해도 꾹 참아야 함
- 우선 순위를 변경하고 나면 플랫폼에 종속적인 부부이 많아지며, 따라서 활동성 문제를 일으키기 쉬움
- 대부분의 병렬 애플리케이션은 모든 스레드의 우선 순위에 기본값을 사용하고 있음

### 10.3.2 형편 없는 응답성
- 소모 상황보다 약간 나은 경우는 바로 응답성이 떨어지는 상황
	- 응답성이 떨어지는 경우는 백그라운드 스레드를 사용하는 GUI 애플리케이션에서 굉장히 일상적임
- 만약 스레드의 우선순위를 조절해야 하는 부분이 있다면, 백그라운드에서 실행되고 있는 기능이 CPU를 많이 사용해 응답성을 저해하는 부분이 해당됨
- 특정 스레드에서 동작하는 기능이 백그라운드로 동작하는 게 효율적이라면, 우선순위를 낮춰 화면 응답성을 높여줄 수 있음
- 애플리케이션 응답성이 떨어진다면 락을 제대로 관리하지 못하는 것이 원인일 수 있음

### 10.3.3 라이브락
- 대기 중인 상태가 아니었다해도 특정 작업의 결과를 받아와야 다음 단계로 넘어갈 수 있는 작업이 실패할 수 밖에 없는 기능을 계속해서 재시도하는 경우에 쉽게 찾아 볼 수 있음
- 메시지를 제대로 전송하지 못했을 때 해당 전송 트랙잭션을 롤백하고 실패한 메시지를 큐의 맨뒤에 쌓아두는 트랜잭션 메시지 전송 애플리케이션에서 자주 발생함
- 메시지를 처리하는 스레드가 대기상태에 들어가지는 않았지만, 다음 작업으로 진행하지도 못하는 상태에 빠진 것
- 에러를 너무 완벽하게 처리하고자 회복 불가능한 오유를 회복 가능하다고 판단해 계속해서 재시도하는 과정에서 나타남
- 여러 스레드가 함께 동작하는 환경에서 각 스레드가 다른 스레드의 응답에 따라 각자의 상태를 계속해서 변경하느라 실제 작업은 전혀 진전시키지 못하는 경우에도 발생함
	- 라이브락을 해결하려면 작업을 재시도하는 부분에 약간의 규칙적이지 않는 구조를 넣어두면 됨
